...
**How many digits.** `Digits:` sits beside `Units:` and answers "these two revisions both read 2.01 nH and I need the fourth figure". `default` is what the tool has always printed — 3 significant digits in the smart units mode, 4 in aligned and for Q, k and dB — and `3` … `8` mean that many everywhere: the table, the coupling blocks, all three views, the Z matrix and the plot's cursor readout, together, so one screen never shows two precisions. Nothing is recomputed, the columns widen to fit, and every open run page is repainted. The ceiling is 8 because every value here is the end of a Schur reduction and a pseudo-inverse — digits past the eighth belong to the arithmetic, not the measurement — and because each one widens the readout box on the plot. Export CSV is unaffected: it has always written full precision. The choice is saved with the config, like the units mode and the view.
...
Reciprocity: OK -- reciprocal (7.18e-16)
  error = 7.18e-16   (max|Z_ab - Z_ba| / max|Z_ab| over the finite off-diagonal entries)
  ...
  legend: ind = Im(Z)>0 (inductive) | cap = Im(Z)<0 (capacitive; past SRF for an inductor) | R<0 = non-passive | |k|>1 = check the port setup
...
The pair list is **ranked strongest-first by `worst M/L`** — the rank key, printed on
the headline — and anything below −60 dB is folded into one counted line that points
at `--csv`, which has no floor. With one pair there is nothing to rank, so the heading
does not say so.
